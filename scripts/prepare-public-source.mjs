...
const DOCS = new Set(['INSTALLATION.md', 'TEST-BETA.md', 'BETA-MAC.md', 'VPS.md', 'SECURITY.md', 'PRIVACY.md', 'PUBLICATION.md', 'PROTOCOL.md', 'SITE-PRESENTATION.md', 'VALIDATION-0.2.0.md', 'VALIDATION-0.2.1.md', 'SIGNATURES.md', 'UPDATES.md', 'agent-world-updater.pub', 'agent-world-release.pub', 'allowed-signers', 'asset-rights.json', 'dependency-rights.json', 'dependency-licenses.json']);
...
export function isPublicSource(file) {
  ...
  if (['.github/workflows/beta.yml', '.github/workflows/checks.yml'].includes(file)) return true;
  if (/^src\/.+\.(?:tsx?|css|png)$/.test(file)) return true;
  if (/^scripts\/[^/]+\.(?:mjs|sh|swift|py)$/.test(file)) return true;
  ...
  return /^(?:collector|reader)\/(?:Cargo\.(?:toml|lock)|(?:src|tests)\/[^/]+\.rs)$/.test(file);
}
