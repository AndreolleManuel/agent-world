...
function show(service: UpdateBackend) { render(<StrictMode><UpdateProvider backend={service}><UpdateControl /></UpdateProvider></StrictMode>); }
beforeEach(() => {
...
it('checks once under StrictMode but never installs automatically; displays notes as text', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'Mise à jour 0.3.0 disponible' }));
...
it('respects disabled startup checks and persists the preference', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Mises à jour' }));
  await screen.findByText('Version installée : 0.2.0'); expect(service.check).not.toHaveBeenCalled();
...
it('prevents concurrent clicks and restores focus after closing', async () => {
  ...
  const trigger = screen.getByRole('button', { name: 'Mises à jour' }); trigger.focus(); fireEvent.click(trigger);
  const button = screen.getByRole('button', { name: 'Vérifier les mises à jour' }); fireEvent.click(button); fireEvent.click(button);
...
it('does not contact the network when the build has no publishing key', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Mises à jour' })); await screen.findByText(/ajout de la clé publique/);
  expect(service.check).not.toHaveBeenCalled(); expect(screen.getByRole('button', { name: 'Vérifier les mises à jour' })).toBeDisabled();
});
